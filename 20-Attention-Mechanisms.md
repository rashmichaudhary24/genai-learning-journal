...
**Cross-attention** extends the same idea by allowing one sequence to attend to another.

For example, during machine translation:

![Cross-Attention Matrix](images/cross-attention-matrix.png)

The decoder generates the translated sentence while attending to the encoder's representation of the source sentence.

Unlike self-attention, the two axes of the attention matrix represent different sequences.
